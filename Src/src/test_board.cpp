...
int main()
{
  ...
  std::map<RR::Robot, Sommet>::iterator itr_map2;
  ...
  for (itr_map = g.map.begin(); itr_map != g.map.end(); itr_map++)
  {
    std::cout << "Sommet :"
              ...
              << itr_map->second.parcours << " - "
              << &itr_map->second;
    if (itr_map->second.parent != nullptr)
    {
      std::cout << " - Parent: "
                << itr_map->second.parent->robot.location.line << ":"
                << itr_map->second.parent->robot.location.column << " - "
                << &itr_map->second << ":" << itr_map->second.parent;
    }
    std::cout << std::endl;
  }

  if ((itr_map2 = g.map.find(r)) != g.map.end())
    if ((itr_map = g.map.find(start)) != g.map.end())
      g.parcours(itr_map->second, &itr_map2->second);

  std::cout << &itr_map->second << std::endl;
